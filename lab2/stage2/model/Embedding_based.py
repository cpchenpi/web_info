...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        r_mul_h = None                                                                      # (kg_batch_size, relation_dim)
        r_mul_pos_t = None                                                                  # (kg_batch_size, relation_dim)
        r_mul_neg_t = None                                                                  # (kg_batch_size, relation_dim)
        ...
        pos_score = torch.pow(torch.norm(r_embed + r_mul_h - r_mul_pos_t, dim = 1), 2)  # (kg_batch_size)
        neg_score = torch.pow(torch.norm(r_embed + r_mul_h - r_mul_neg_t, dim = 1), 2)  # (kg_batch_size)
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        pos_score = torch.pow(torch.norm(r_embed + h_embed - pos_t_embed, dim = 1), 2)  # (kg_batch_size)
        neg_score = torch.pow(torch.norm(r_embed + h_embed - neg_t_embed, dim = 1), 2)  # (kg_batch_size)
